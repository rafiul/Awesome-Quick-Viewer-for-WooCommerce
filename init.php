 ...
 * License: GPL2+
 *
 * @package Awesome Quick Viewer for WooCommerce
 */
...
register_activation_hook(__FILE__, function(){
	...
	if( is_plugin_inactive($lite) ){
		add_action('update_option_active_plugins', function($lite){
			deactivate_plugins('awsome-woo-quick-viewer-pro/init.php');
		});
...
// Define constants
define( 'AWQV_VERSION', '1.0' );
define( 'AWQV_DIR', untrailingslashit( plugin_dir_path( __FILE__ ) ) );
define( 'AWQV_INC_DIR', untrailingslashit( plugin_dir_path( __FILE__ ) . 'includes' ) );
define( 'AWQV_PATH', untrailingslashit( plugins_url( basename( plugin_dir_path( __FILE__ ) ), basename( __FILE__ ) ) ) );
define( 'AWQV_CUSTOMIZER_PATH', untrailingslashit( plugins_url( basename( plugin_dir_path( __FILE__ ) ), basename( __FILE__ ) ) . '/includes/cutomizer-controls/controls' ) );
define( 'AWQV_BASE', plugin_basename( __FILE__ ) );
...
function awqv_lite_load_scripts() {
	...
	wp_enqueue_style( 'font_icon_css', awqv_lite_assets_path() . '/icon-picker/assets/css/fontello.css', AWQV_VERSION);
