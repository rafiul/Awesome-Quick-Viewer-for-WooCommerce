...
if (!defined( 'ABSPATH')) exit;
 class Awqv_Lite_front{
	 ...
	 function __construct(){
		...
		add_action( 'wp_ajax_get_product', array($this,'awqv_get_product_callback') );
		// If you want not logged in users to be allowed to use this function as well, register it again with this function:
		add_action( 'wp_ajax_nopriv_get_product', array($this,'awqv_get_product_callback') );
		add_action( 'awqv_before_modal', array($this,'awqv_before_modal', 20 ));
		add_action( 'awqv_after_modal', array($this,'awqv_after_modal', 20 ) );
		add_action( 'wp_footer',  array($this,'awqv_add_ajax_script' ) );
		add_action( 'wp_footer', array($this,'awqv_modal' ));
		add_action( 'woocommerce_after_shop_loop_item', array($this,'awqv_modal_button' ));
		...
		add_action('wp_enqueue_scripts', array($this,'awqv_woocommerce_ajax_add_to_cart_js'));
		add_action('wp_ajax_woocommerce_ajax_add_to_cart', array($this,'awqv_woocommerce_ajax_add_to_cart'));
		add_action('wp_ajax_nopriv_woocommerce_ajax_add_to_cart', array($this,'awqv_woocommerce_ajax_add_to_cart'));
		
		add_action( 'awqv_view_product_image', array( $this, 'awqv_image' ), 20 );
	 }
...
// add the action 
	...
	public function awqv_woocommerce_ajax_add_to_cart_js() {
		...
		if (function_exists('is_product') && is_product()) {
			wp_enqueue_script('woocommerce-ajax-add-to-cart', plugin_dir_url(__FILE__) . 'assets/js/woo-ajax-add-to-cart.js', array('jquery'), '', true);
		}
	}
